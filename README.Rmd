...
Finally, `DS_helper()` will use the above functions to validate and format inputs into datastreamr endpoints. It will also split filters into separate queries if too many are specified, and iterate over values of `$top` in instances where the resulting query is too large.
